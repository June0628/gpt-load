...
async function handleClearTask() {
  try {
    await keysApi.clearTask();
    visible.value = false;
    taskInfo.value = { is_running: false, task_type: "KEY_VALIDATION" };
    message.success(t("task.taskCleared"));
  } catch (error) {
    ...
    <div class="progress-container">
      <div class="progress-header">
        ...
        <n-button
          ...
          circle
          size="small"
          @click="handleClearTask"
          :title="t('task.clearTask')"
          type="warning"
        >
          <template #icon>
