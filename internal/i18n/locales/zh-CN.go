...
var MessagesZhCN = map[string]string{
	...
	"config.db_size_monitor_enabled":             "启用数据库大小监控",
	"config.db_size_monitor_enabled_desc":        "定时检查数据库总大小，超过阈值时发送飞书通知。",
	"config.db_size_monitor_threshold_gb":        "数据库大小阈值（GB）",
	"config.db_size_monitor_threshold_gb_desc":   "触发告警的数据库总大小阈值，单位为 GB。",
	"config.db_size_monitor_interval_hours":      "数据库大小检查间隔（小时）",
	"config.db_size_monitor_interval_hours_desc": "定时检查数据库大小的间隔时间，单位为小时。",
	...
	"config.balance_query_interval":           "余额查询间隔（分钟）",
	"config.balance_query_interval_desc":      "后台查询密钥余额的默认间隔（分钟），独立于密钥验证间隔。",
	...
	"config.category.log_upload": "日志上传",

	// Log upload related
	"config.log_upload_enabled":                  "启用日志自动上传",
	"config.log_upload_enabled_desc":             "启用后，日志将在删除前自动上传到外部存储。",
	"config.log_upload_before_delete":            "删除前上传",
	"config.log_upload_before_delete_desc":       "在日志被清理删除之前自动上传到外部存储。",
	"config.log_upload_directory":                "上传目录",
	"config.log_upload_directory_desc":           "在外部存储中保存日志的目录路径。",
	"config.log_upload_filename_prefix":          "文件名前缀",
	"config.log_upload_filename_prefix_desc":     "上传文件名的前缀标识。",
	"config.log_upload_provider":                 "上传服务商",
	"config.log_upload_provider_desc":            "选择日志上传的目标服务商：tencent（腾讯云 COS）或 webdav。",
	"config.log_upload_tencent_secret_id":        "腾讯云 SecretId",
	"config.log_upload_tencent_secret_id_desc":   "腾讯云 COS 的 SecretId 凭证。",
	"config.log_upload_tencent_secret_key":       "腾讯云 SecretKey",
	"config.log_upload_tencent_secret_key_desc":  "腾讯云 COS 的 SecretKey 凭证。",
	"config.log_upload_tencent_bucket":           "存储桶名称",
	"config.log_upload_tencent_bucket_desc":      "腾讯云 COS 存储桶名称。",
	"config.log_upload_tencent_region":           "存储桶地域",
	"config.log_upload_tencent_region_desc":      "腾讯云 COS 存储桶所在地域，如 ap-guangzhou。",
	"config.log_upload_webdav_url":               "WebDAV 地址",
	...
	"log_backup.table_empty":               "该日志表为空，无需上传",
