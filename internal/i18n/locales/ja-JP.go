...
var MessagesJaJP = map[string]string{
	...
	"config.balance_query_interval":           "残高照会間隔（分）",
	"config.balance_query_interval_desc":      "バックグラウンドで残高を照会するデフォルト間隔（分）。鍵検証間隔とは独立しています。",
	...
	"config.category.log_upload": "ログアップロード",

	// Log upload related
	"config.log_upload_enabled":                  "自動アップロードを有効化",
	"config.log_upload_enabled_desc":             "有効にすると、ログは削除前に外部ストレージに自動的にアップロードされます。",
	"config.log_upload_before_delete":            "削除前にアップロード",
	"config.log_upload_before_delete_desc":       "ログがクリーンアップされる前に外部ストレージに自動的にアップロードします。",
	"config.log_upload_directory":                "アップロードディレクトリ",
	"config.log_upload_directory_desc":           "外部ストレージにログを保存するディレクトリパス。",
	"config.log_upload_filename_prefix":          "ファイル名プレフィックス",
	"config.log_upload_filename_prefix_desc":     "アップロードファイル名のプレフィックス識別子。",
	"config.log_upload_provider":                 "アップロードプロバイダー",
	"config.log_upload_provider_desc":            "ログアップロードの対象プロバイダーを選択：tencent（騰訊雲 COS）または webdav。",
	"config.log_upload_tencent_secret_id":        "騰訊雲 SecretId",
	"config.log_upload_tencent_secret_id_desc":   "騰訊雲 COS の SecretId 認証情報。",
	"config.log_upload_tencent_secret_key":       "騰訊雲 SecretKey",
	"config.log_upload_tencent_secret_key_desc":  "騰訊雲 COS の SecretKey 認証情報。",
	"config.log_upload_tencent_bucket":           "バケット名",
	"config.log_upload_tencent_bucket_desc":      "騰訊雲 COS バケット名。",
	"config.log_upload_tencent_region":           "バケットリージョン",
	"config.log_upload_tencent_region_desc":      "騰訊雲 COS バケットの所在地域（例：ap-guangzhou）。",
	...
	"log_backup.table_empty":               "ログテーブルが空のため、アップロードは不要です",
